...
def generate_combinations():
    ...
    for combination in itertools.permutations(other_words):
        yield list(combination[:2]) + [fixed_word] + list(combination[2:]), other_words
    ...
    with Manager() as manager:
        ...
        with ProcessPoolExecutor(max_workers=cpu_count()) as executor:
            ...
            for _ in range(50000):  # Limite de 50.000 combinações por execução
                try:
                    combination, other_words = next(combination_generator)
            ...
            with open(all_combinations_file, "w") as f:
                json.dump(dict(all_combinations), f, indent=4)
